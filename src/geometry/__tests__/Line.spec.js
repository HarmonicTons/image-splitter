...
describe('isOnLine', () => {
  ...
  it('should return true when a point is a corner of the line with a threshold', () => {
    const line = new Line({
      x0: 1, y0: 0, x1: 5, y1: 2,
    });
    const point = new Point({ x: 0.99, y: -0.01 });
    expect(line.isOnLine({ point, oob: false, threshold: 0.1 })).toBe(true);
  });
});
...
describe('intersection', () => {
  ...
  it('should return a point when two lines intersect, one being horizontal', () => {
    const line1 = new Line({
      x0: 3, y0: 3, x1: 6, y1: 3,
    });
    const line2 = new Line({
